...
transformed parameters {
  ...
  vector[PAR_size] PAR;
  vector[PAR_size_] PAR_; // = link(par)
  ...
  for(g in 1:PAR_group_max_%)PAR_random_%[g] = to_matrix(diag_pre_multiply(PAR_sd_%_, L_corr_PAR_%) * PAR_z_%[g], PAR_size, Z_PAR_ncol_%); //random-PAR
  ...
  for(n in 1:N){
    ...
    PAR = PAR + PAR_random_%[PAR_group_%[n]] * Z_PAR_%[n]';  //random-PAR
...
model {
  for(i in 1:PAR_size)for(j in 1:X_PAR_ncol)PAR_fixed[i, j] ~ normal(PAR_prior_fixed_mu[i, j], PAR_prior_fixed_sd[i, j]);
  for(i in 1:PAR_size)for(j in 1:Z_PAR_ncol_%){ PAR_sd_%[i, j] ~ cauchy(0, PAR_prior_scale_%[i, j]); } //random-PAR

  L_corr_PAR_% ~ lkj_corr_cholesky(PAR_prior_nu_%); //random-PAR

  for(g in 1:PAR_group_max_%){ PAR_z_%[g] ~ normal(0, 1); } //random-PAR
